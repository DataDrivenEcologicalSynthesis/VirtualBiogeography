...
Tspecies<-c("Pelecanus occidentalis") #creating an object called focalspecies with a list of our focal species
...
Tsp<-occ_search(taxonKey=paste0(Tkeys, collapse=";"),hasCoordinate = TRUE, hasGeospatialIssue = FALSE, limit=1000) #extracting occurrence records for focal species - only getting recrods with lat/long and without geospatial issues. 
...
write.csv(Tdf, "Databases/GBIFOccurrences/PelicanGBIFFULL.csv") #all columns saved

focused<-focused<-subset(Tdf[, 2:4]) #subsetting the original dataframe to just contain species names and lat/longs

write.csv(focused, "Databases/GBIFOccurrences/PelicanGBIFMinimal.csv") #csv file of occurrences by species names and lat/longs only
